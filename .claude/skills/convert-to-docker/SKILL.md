...
Yes. Guardian Core uses Docker, which works on both macOS and Linux. Just install Docker and run `/setup`.
```
...
\`\`\`bash
docker images | grep guardian-core-agent
...
docker run --rm --entrypoint /bin/bash -v /tmp/test-ro:/test:ro guardian-core-agent:latest \
...
```bash
mkdir -p /tmp/test-rw
docker run --rm --entrypoint /bin/bash -v /tmp/test-rw:/test guardian-core-agent:latest \
