...
import {
	type NewMessage,
	PhoneContacts as PhoneContactsSchema,
...
function resolveContact(contactId?: string): {
	...
	try {
		if (!fs.existsSync(PHONE_CONTACTS_PATH)) {
			logger.error({ path: PHONE_CONTACTS_PATH }, 'Phone contacts file not found — calls disabled');
			return null;
		}
		...
		const defaultExists = contacts.contacts.some((c) => c.id === contacts.default_contact);
		if (!defaultExists) {
			logger.error(
		...
		if (!contact) {
			...
			logger.warn({ contact_id: targetId }, 'Contact not found in allowlist');
			return null;
		}
...
function migrateAlertPhoneNumber(): void {
	...
	if (!alertNumber) {
		logger.warn(
			'Phone calls disabled — no contacts file at ' +
				PHONE_CONTACTS_PATH +
	...
	logger.info({ path: PHONE_CONTACTS_PATH }, 'Migrated ALERT_PHONE_NUMBER to phone-contacts.json');
}
	...
	switch (data.type) {
		case 'schedule_task':
			if (data.prompt && data.schedule_type && data.schedule_value && data.groupFolder) {
				...
				const taskId = TaskId.make(`task-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
				const contextMode =
		...
		case 'phone_call':
			if (!isMain) {
				logger.warn({ sourceGroup }, 'Unauthorized phone_call attempt blocked');
				break;
			}
			if (data.reason) {
				...
				await makeOutboundCall(data.reason, contact.number, contact.class, contact.name);
				logger.info(
					{
						reason: data.reason,
...
function ensureDockerRunning(): void {
	...
	try {
		const output = execSync('docker ps -a --filter name=guardian-core- --format {{.Names}}', {
