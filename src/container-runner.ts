/**
 * ContainerRunnerService — Effect port of container-runner.ts
 *
...
export class ContainerRunnerService extends Context.Tag(
  'ContainerRunnerService',
)<ContainerRunnerService, ContainerRunnerServiceShape>() {}
...
const makeContainerRunnerService = Effect.sync(() => {
  const runContainerAgentFn: ContainerRunnerServiceShape['runContainerAgent'] = (
    group,
    input,
  ) =>
    Effect.gen(function* () {
      ...
      const containerName = `guardian-core-${safeName}-${Date.now()}`;
  ...
  const writeGroupsSnapshotFn: ContainerRunnerServiceShape['writeGroupsSnapshot'] =
    (groupFolder, isMain, groups, _registeredJids) =>
      Effect.sync(() => {
        const groupIpcDir = path.join(DATA_DIR, 'ipc', groupFolder);
        fs.mkdirSync(groupIpcDir, { recursive: true });
...
// --- Legacy wrappers (drop-in replacement for original container-runner.ts exports) ---

const legacyService = Effect.runSync(makeContainerRunnerService);
  ...
  try {
    return await Effect.runPromise(
      legacyService.runContainerAgent(group, input).pipe(
        Effect.catchAll((err: ContainerSpawnError | ContainerTimeoutError | ContainerExitError | ContainerOutputParseError) => {
          // Map tagged errors to ContainerOutput { status: 'error' } (preserving never-throw behavior)
          let errorMessage: string;
          switch (err._tag) {
            case 'ContainerTimeoutError':
              errorMessage = `Container timed out after ${err.timeoutMs}ms`;
              break;
            case 'ContainerExitError':
              errorMessage = `Container exited with code ${err.exitCode}: ${err.stderr ?? ''}`;
              break;
            case 'ContainerSpawnError':
            case 'ContainerOutputParseError':
              errorMessage = `${err._tag}: ${err.message}`;
              break;
          }
...
export function writeGroupsSnapshot(
  ...
  Effect.runSync(
    legacyService.writeGroupsSnapshot(
      groupFolder,
      isMain,
      groups,
      registeredJids,
