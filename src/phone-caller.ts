/**
 * PhoneCallerService — Effect port of phone-caller.ts
 *
...
/**
 * Load a specific env var from the project .env file.
 * Falls back to process.env if not found in .env.
 */
...
export function sanitizeReason(reason: string): string {
  let sanitized = reason.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');
...
export function loadVoicePrompt(
  ...
  try {
    ...
    if (template.length < 10) {
      logger.warn(
        { voicePromptPath },
        'VOICE_PROMPT.md appears empty, using fallback',
      );
    ...
    logger.warn(
      { voicePromptPath },
      'VOICE_PROMPT.md not found, using fallback',
    );
    template = FALLBACK_VOICE_PROMPT;
  }
  ...
  if (contactClass === 'owner') {
    classInstructions = OWNER_CONTACT_CLASS_INSTRUCTIONS;
  } else {
    classInstructions = THIRD_PARTY_CONTACT_CLASS_INSTRUCTIONS_TEMPLATE.replace(
      '{{name}}',
      contactName,
    );
  }
...
export function buildFirstMessage(
  ...
  if (contactClass === 'owner') {
    return `Hey, this is Rumi. I'm calling because: ${sanitized}. I've sent the full details to your WhatsApp. Do you have any questions?`;
  }

  return `Hi, this is Rumi calling on behalf of Shovon. I'm reaching out because: ${sanitized}.`;
}
...
const makePhoneCallerService = Effect.sync(() => {
  /**
  ...
  const pollAndSaveTranscript = (
    ...
    Effect.gen(function* () {
      logger.info({ conversationId }, 'Polling for call transcript...');
      ...
      const pollOnce = Effect.gen(function* () {
        ...
        if (!response.ok) {
          logger.warn(
            ...
            'Failed to poll conversation status',
          );
        ...
        if (data.status === 'done') {
          if (!data.transcript || data.transcript.length === 0) {
            logger.warn(
              { conversationId },
              'Call ended but no transcript available',
            );
            return 'done' as const;
          }
          ...
          lines.push(
            `Duration: ${data.metadata?.call_duration_secs || 'unknown'}s`,
          );
          ...
          const ipcData = {
            ...
            schedule_type: 'once',
            schedule_value: new Date(Date.now() + 10000).toISOString(),
          ...
          const ipcFilename = `${Date.now()}-phone-transcript.json`;
        ...
        Effect.catchAll((err) => {
          logger.warn(
            { err, conversationId },
            'Error polling conversation status',
          );
      ...
      yield* pollOnce.pipe(
        ...
        Effect.catchAll(() => {
          logger.warn(
            { conversationId },
            'Gave up polling for transcript after max attempts',
          );
          return Effect.void;
        }),
      );
  ...
  const makeOutboundCallFn: PhoneCallerServiceShape['makeOutboundCall'] = (
    ...
    Effect.gen(function* () {
      ...
      const agentId = getEnvVar('ELEVENLABS_AGENT_ID');
      const phoneNumberId = getEnvVar('ELEVENLABS_PHONE_NUMBER_ID');
      ...
      if (result.conversation_id) {
        ...
        logger.warn(
          'No conversation_id returned from outbound call — transcript capture skipped',
        );
      }
    }).pipe(
...
export const PhoneCallerServiceLive = Layer.effect(
  PhoneCallerService,
  makePhoneCallerService,
);

// --- Legacy wrapper (drop-in replacement for original phone-caller.ts export) ---

const legacyService = Effect.runSync(makePhoneCallerService);
...
export async function makeOutboundCall(
  ...
  try {
    await Effect.runPromise(
      legacyService
        .makeOutboundCall(reason, toNumber, contactClass, contactName)
        .pipe(
          Effect.catchAll((err) => {
