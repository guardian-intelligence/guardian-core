  ...
  return Layer.succeed(ShellService, {
    run: (cmd, args) =>
      Effect.gen(function* () {
        ...
        Effect.withLogSpan('shell.run'),
        Effect.annotateLogs({ cmd, args: args.join(' ') }),
      ),
  });
}
...
function makeDarwinPlatform(shell: ShellServiceShape): PlatformServiceShape {
  ...
  const plistDst = path.join(
    home,
    'Library',
    'LaunchAgents',
    'com.guardian-core.plist',
  );
  ...
  return {
    ...
    verifyService: Effect.gen(function* () {
      ...
      if (pid && pid !== '-') {
        ...
        yield* warn(
          'Service loaded but not running yet — check logs/guardian-core.error.log',
        );
      }
...
function makeLinuxPlatform(shell: ShellServiceShape): PlatformServiceShape {
  ...
  return {
    ...
    verifyService: Effect.gen(function* () {
      yield* Effect.sleep('2 seconds');
      yield* info('Verifying service...');
      const status = yield* shell.run('systemctl', [
        '--user',
        'is-active',
        'guardian-core',
      ]);
      if (status.trim() === 'active') {
        ...
        yield* warn(
          `Service status: ${status.trim()} — check logs/guardian-core.error.log`,
        );
      }
    }).pipe(Effect.withLogSpan('platform.verifyService')),
  };
  ...
  return Effect.gen(function* () {
      ...
      if (allChanges.length === 0) {
        ...
        if (fs.existsSync(distIndex)) {
          ...
          needApp = srcFiles.some(
            (f) => fs.statSync(f).mtimeMs > distStat.mtimeMs,
          );
        } else {
          needApp = true;
        }
  ...
  Effect.gen(function* () {
    ...
    return {
      deploy: (mode, dryRun) =>
        Effect.gen(function* () {
          // 1. Detect changes
          const { needApp, needContainer } = yield* detectChanges(
            shell,
            mode,
          );
          ...
          if (needApp)
            yield* Effect.log(
              '  • Rebuild host app (bun install → typecheck → test → build)',
            );
          if (needContainer)
            yield* Effect.log('  • Rebuild container image (docker build)');
          ...
          if (needContainer) {
            yield* Effect.gen(function* () {
              yield* info(
                'Building container image (this may take a minute)...',
              );
          ...
          yield* Effect.log(
            `  Logs: tail -f ${projectRoot()}/logs/guardian-core.log`,
          );
        }).pipe(
          Effect.withLogSpan('deploy'),
          Effect.annotateLogs({ mode, dryRun }),
        ),
    };
  }),
);
