...
import type { NewMessage, ScheduledTask, TaskRunLog } from './schemas.js';
...
export interface DatabaseServiceShape {
	...
	readonly getLastGroupSync: Effect.Effect<Option.Option<string>, DatabaseQueryError>;
	...
	readonly getNewMessages: (
		...
		botPrefix: string,
	) => Effect.Effect<{ messages: NewMessage[]; newTimestamp: string }, DatabaseQueryError>;
	...
	readonly getTaskById: (
		id: string,
	) => Effect.Effect<Option.Option<ScheduledTask>, DatabaseQueryError>;
	...
	readonly deleteTask: (id: string) => Effect.Effect<void, DatabaseQueryError>;
	...
	readonly logTaskRun: (log: TaskRunLog) => Effect.Effect<void, DatabaseQueryError>;
...
const makeDatabaseService = Effect.gen(function* () {
	...
	const initDatabaseFn: DatabaseServiceShape['initDatabase'] = Effect.gen(function* () {
		yield* Effect.try({
			try: () => {
	...
	const storeChatMetadataFn: DatabaseServiceShape['storeChatMetadata'] = (
		...
		}).pipe(Effect.annotateLogs({ chatJid }), Effect.withLogSpan('db.storeChatMetadata'));

	const updateChatNameFn: DatabaseServiceShape['updateChatName'] = (chatJid, name) =>
		Effect.try({
			try: () => {
				db.prepare(
					`
		...
		}).pipe(Effect.annotateLogs({ chatJid }), Effect.withLogSpan('db.updateChatName'));
	...
	const getLastGroupSyncFn: DatabaseServiceShape['getLastGroupSync'] = Effect.try({
		try: () => {
			const row = db
				.prepare(`SELECT last_message_time FROM chats WHERE jid = '__group_sync__'`)
				.get() as { last_message_time: string } | undefined;
			return Option.fromNullable(row?.last_message_time || null);
		},
		catch: (err) =>
			new DatabaseQueryError({
	...
	const setLastGroupSyncFn: DatabaseServiceShape['setLastGroupSync'] = Effect.try({
		try: () => {
			const now = getNowIso();
			db.prepare(
	...
	const storeMessageFn: DatabaseServiceShape['storeMessage'] = (
		...
		}).pipe(Effect.annotateLogs({ chatJid }), Effect.withLogSpan('db.storeMessage'));
	...
	const getMessagesSinceFn: DatabaseServiceShape['getMessagesSince'] = (
		...
		}).pipe(Effect.annotateLogs({ chatJid }), Effect.withLogSpan('db.getMessagesSince'));
	...
	const createTaskFn: DatabaseServiceShape['createTask'] = (task) =>
		...
		}).pipe(Effect.annotateLogs({ taskId: task.id }), Effect.withLogSpan('db.createTask'));
	...
	const getTasksForGroupFn: DatabaseServiceShape['getTasksForGroup'] = (groupFolder) =>
		Effect.try({
			try: () =>
				db
	...
	const getAllTasksFn: DatabaseServiceShape['getAllTasks'] = Effect.try({
		try: () =>
			db.prepare('SELECT * FROM scheduled_tasks ORDER BY created_at DESC').all() as ScheduledTask[],
		catch: (err) =>
			new DatabaseQueryError({
	...
	const updateTaskFn: DatabaseServiceShape['updateTask'] = (id, updates) =>
		...
		}).pipe(Effect.annotateLogs({ taskId: id }), Effect.withLogSpan('db.updateTask'));
	...
	const deleteTaskFn: DatabaseServiceShape['deleteTask'] = (id) =>
		...
		}).pipe(Effect.annotateLogs({ taskId: id }), Effect.withLogSpan('db.deleteTask'));
	...
	const updateTaskAfterRunFn: DatabaseServiceShape['updateTaskAfterRun'] = (
		...
		}).pipe(Effect.annotateLogs({ taskId: id }), Effect.withLogSpan('db.updateTaskAfterRun'));
	...
	const logTaskRunFn: DatabaseServiceShape['logTaskRun'] = (log) =>
		...
		}).pipe(Effect.annotateLogs({ taskId: log.task_id }), Effect.withLogSpan('db.logTaskRun'));

	const getTaskRunLogsFn: DatabaseServiceShape['getTaskRunLogs'] = (taskId, limit = 10) =>
		Effect.try({
			try: () =>
				db
					.prepare(
