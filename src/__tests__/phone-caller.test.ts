...
describe('phone-caller', () => {
	...
	describe('PhoneContacts schema', () => {
		...
		it('should reject invalid E.164 numbers', () => {
			const invalid = {
				...VALID_CONTACTS,
				contacts: [{ id: 'bad', name: 'Bad', number: '5551234567', class: 'owner' }],
			};
			expect(() => Schema.decodeUnknownSync(PhoneContactsSchema)(invalid)).toThrow();
		});

		it('should reject empty contact id', () => {
			const invalid = {
				...VALID_CONTACTS,
				contacts: [{ id: '', name: 'Empty', number: '+15551234567', class: 'owner' }],
			};
			expect(() => Schema.decodeUnknownSync(PhoneContactsSchema)(invalid)).toThrow();
		});
		...
		it('should reject invalid contact class', () => {
			const invalid = {
				...VALID_CONTACTS,
				contacts: [{ id: 'x', name: 'X', number: '+15551234567', class: 'admin' }],
			};
			expect(() => Schema.decodeUnknownSync(PhoneContactsSchema)(invalid)).toThrow();
		});
...
Do NOT update USER.md with observations about this person.`;
		...
		it('should contain "update USER.md" for owner calls', () => {
			const prompt = buildTestFollowUpPrompt('2026-02-06-phone-call-1430.txt', 'owner', 'Shovon');
