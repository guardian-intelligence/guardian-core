...
import { DeployService, DeployServiceLive, PlatformService, ShellService } from '../deploy.js';
...
function makePlatformServiceTest(platform: 'darwin' | 'linux' = 'darwin'): {
	layer: Layer.Layer<PlatformService>;
	calls: string[];
} {
	const calls: string[] = [];
	const layer = Layer.succeed(PlatformService, {
		platform,
		installServiceTemplate: Effect.sync(() => {
			calls.push('installServiceTemplate');
		}),
		restartService: Effect.sync(() => {
			calls.push('restartService');
		}),
		verifyService: Effect.sync(() => {
...
describe('DeployService', () => {
	it('should detect "all" mode and run full pipeline', async () => {
		...
		const cmds = shell.commands.map((c) => `${c.cmd} ${c.args.join(' ')}`.trim());
		expect(cmds).toContainEqual('bun install');
		...
		expect(platform.calls).toEqual(['installServiceTemplate', 'restartService', 'verifyService']);
	});
	...
	it('should only run container step in "container" mode', async () => {
		...
		const cmds = shell.commands.map((c) => `${c.cmd} ${c.args.join(' ')}`.trim());
		expect(cmds).not.toContainEqual('bun install');
